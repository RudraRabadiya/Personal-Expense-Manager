...
import { LABELS, fmt } from '../lib/utils'
...
export default function AddEntryModal({ type: defaultType = 'expense', onClose, onSuccess }) {
  const [type, setType] = useState(defaultType)
  const [form, setForm] = useState({ amount: '', description: '', category: 'General', date: new Date().toISOString().slice(0,10), notes: '' })
    ...
    <div className="overlay" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal">
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 18 }}>
          {[['expense', LABELS.expense, 'var(--red)', '#ef444418'], ['income', LABELS.income, 'var(--green)', '#22c55e18']].map(([t, label, color, bg]) => (
        ...
        <div className="field"><label>Description</label><input value={form.description} onChange={e=>setForm({...form,description:e.target.value})} placeholder="e.g. Grocery, Salary..." /></div>
        ...
        <div className="field"><label>Notes (optional)</label><input value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder="Extra info..." /></div>
